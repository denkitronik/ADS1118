...
*/
union Config {
	struct {					//BIT MAPPING WITH ADS1118 CONFIG REGISTER (See [1])
...
class ADS1118 {
    public:
        ...
        const uint8_t FSR_6144    = 0b000;  //LSB SIZE = 187.5μV
        const uint8_t FSR_4096    = 0b001;  //LSB SIZE = 125μV
        const uint8_t FSR_2048    = 0b010;  //LSB SIZE = 62.5μV ***DEFAULT
        const uint8_t FSR_1024    = 0b011;  //LSB SIZE = 31.25μV
        const uint8_t FSR_0512    = 0b100;  //LSB SIZE = 15.625μV
        const uint8_t FSR_0256    = 0b111;  //LSB SIZE = 7.8125μV

        /*Sampling rate selection by "DR" bits. 
		[Warning: this could increase the noise and the effective number of bits (ENOB). See tables above]*/
        const uint8_t RATE_8SPS   = 0b000;  //Tconv=125ms
        const uint8_t RATE_16SPS  = 0b001;  //Tconv=62.5ms
        const uint8_t RATE_32SPS  = 0b010;  //Tconv=31.25ms
        const uint8_t RATE_64SPS  = 0b011;  //Tconv=15.625ms
        const uint8_t RATE_128SPS = 0b100;  //Tconv=7.8125ms
        const uint8_t RATE_250SPS = 0b101;  //Tconv=4ms
        const uint8_t RATE_475SPS = 0b110;  //Tconv=2.105ms
        const uint8_t RATE_860SPS = 0b111;  //Tconv=1.163ms	
